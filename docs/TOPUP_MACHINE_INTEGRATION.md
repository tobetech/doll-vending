...
### แอป — ยกเลิก QR เติมเงิน → n8n

เมื่อผู้ใช้กด **ยกเลิก** บนหน้าแสดง QR (หลังกดตกลง) แอปเรียก **`POST /api/vending/topup-cancel-notify`** (พร้อม `Authorization: Bearer`) แล้วเซิร์ฟเวอร์จะ **`POST` JSON ไปที่ `N8N_TOPUP_CANCEL_WEBHOOK_URL`** (ถ้าตั้งค่าใน env)
...
**JSON ที่ส่งต่อไป n8n (ตัวอย่าง):**

```json
{
  "event": "topup_qr_cancelled",
  "userId": "uuid",
  "token": "uuid",
  "amountBaht": 100,
  "cancelledAt": "2026-03-28T12:00:00.000Z",
  "tokenStatus": "pending",
  "tokenExpiresAt": "...",
  "tokenAmount": 100
}
...
ถ้าไม่ตั้ง `N8N_TOPUP_CANCEL_WEBHOOK_URL` API ยังตอบ `200` พร้อม `skipped: true` — แอปยังปิด QR ได้ตามปกติ
